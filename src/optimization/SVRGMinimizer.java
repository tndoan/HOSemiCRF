...
public class SVRGMinimizer extends Minimizer {
	...
	/**
	 * This function is the experience function of using 
	 * @param f
	 * @param init
	 * @param learningRate
	...
	public double[] minimize(AbstractSVRGFunction f, double[] init,
		...
		int[] i_t = new int[numRandom];
		double nR = (double) numRandom;
			...
			for (int t = 0; t < upFreq; t++){
				...
				for (int i = 0; i < d; i++)
					for (int j = 0; j < numRandom; j++)
						f2[i] += eachDerivatives[i_t[j]][i] ;

				for (int i = 0; i < d; i++){
					w_0[i] = w_0[i] - learningRate * (f1[i] / nR - f2[i] / nR + mu_tilde[i]);
				}
			}
